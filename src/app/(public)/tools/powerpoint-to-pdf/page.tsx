...
export default function PowerpointToPdfPage() {
  ...
  const handleFile = (selected: FileList | null) => {
    ...
    const validExt = selectedFile?.name.endsWith('.ppt') || selectedFile?.name.endsWith('.pptx');
        ...
        {step === 'upload' && (
          <div className="space-y-6">
            <ToolCard>
              ...
              {!file ? (
                <ToolUploadZone icon={Upload} title="Drop a file here" subtitle="or click to browse" accept=".ppt,.pptx,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation" onFiles={handleFile} />
              ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-3">
              ...
              <ToolPrimaryButton onClick={handleProcess} loading={isProcessing}>
                {isProcessing ? (<><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin shrink-0" /><span>Processing...</span></>) : (<><Presentation className="w-5 h-5 shrink-0" /><span>Convert to PDF</span></>)}
              </ToolPrimaryButton>
            </div>
          </div>
        )}

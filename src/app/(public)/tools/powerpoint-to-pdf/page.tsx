...
// Anything LibreOffice Impress can open and export straight to PDF.
const ACCEPTED_EXTENSIONS = ['.ppt', '.pptx', '.pptm', '.pps', '.ppsx', '.pot', '.potx', '.odp', '.otp'];
...
export default function PowerpointToPdfPage() {
  ...
  const handleFile = (selected: FileList | null) => {
    ...
    if (selectedFile && validExt) {
      ...
      setError('Please upload a valid PowerPoint file (.ppt, .pptx or .odp)');
    }
  };

  const handleContinueToOptions = () => {
    if (!file) {
      setError('Please select a presentation to continue');
      return;
    }
    setError(null);
    goToOptions();
  ...
  return (
    <ToolPageShell title="PowerPoint to PDF" description="Convert PowerPoint presentations to PDF." icon={Presentation}>
          ...
          <div className="space-y-6">
            <ToolCard>
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-gray-500">Select a presentation to get started</p>
              </div>
              {!file ? (
                <ToolUploadZone icon={Upload} title="Drop a file here" subtitle="or click to browse (.ppt, .pptx, .odp)" accept={ACCEPTED_EXTENSIONS.join(',')} onFiles={handleFile} />
              ) : (
          ...
          <div className="space-y-6">
            <ToolCard>
              ...
              <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl">
                <div className="p-3 bg-white border border-gray-100 rounded-xl text-brand-red"><Presentation className="w-6 h-6" /></div>
              ...
              <div className="flex items-start gap-2.5 p-3 bg-blue-50 border border-blue-100 rounded-xl mt-4">
                ...
                <p className="text-xs text-blue-700 leading-relaxed">No options needed — every slide is rendered to a PDF page with LibreOffice, keeping images, layout and theme colours.</p>
              </div>
            </ToolCard>
            <div className="flex justify-end gap-3">
              ...
              <ToolPrimaryButton onClick={handleProcess} loading={isProcessing}>
                {isProcessing ? (<><Spinner size={24} color="#ffffff" className="shrink-0" /><span>Processing...</span></>) : (<><Presentation className="w-5 h-5 shrink-0" /><span>Convert to PDF</span></>)}
              </ToolPrimaryButton>
            </div>
          </div>
        )}
        ...
        {step === 'download' && (
          <div className="space-y-6">
            <ToolCard className="text-center py-12 sm:py-16">
              ...
              <h2 className="font-display font-bold text-2xl sm:text-3xl text-brand-dark mb-3">PowerPoint Converted Successfully!</h2>

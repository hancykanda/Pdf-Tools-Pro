...
export default function WordToPdfPage() {
  ...
  const handleContinueToOptions = () => {
    if (!file) {
      setError('Please select a Word document to continue');
      return;
    }
    setError(null);
    goToOptions();
  ...
  return (
    <ToolPageShell title="Word to PDF" description="Convert Word documents to PDF format." icon={FileText}>
          ...
          <div className="space-y-6">
            <ToolCard>
              ...
              <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl">
                <div className="p-3 bg-white border border-gray-100 rounded-xl text-brand-red"><FileText className="w-6 h-6" /></div>
            ...
            <div className="flex justify-end gap-3">
              ...
              <ToolPrimaryButton onClick={handleProcess} loading={isProcessing}>
                {isProcessing ? (<><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin shrink-0" /><span>Processing...</span></>) : (<><FileText className="w-5 h-5 shrink-0" /><span>Convert to PDF</span></>)}
              </ToolPrimaryButton>
            </div>
          </div>
        )}
        ...
        {step === 'download' && (
          <div className="space-y-6">
            <ToolCard className="text-center py-12 sm:py-16">
              ...
              <h2 className="font-display font-bold text-2xl sm:text-3xl text-brand-dark mb-3">Word Document Converted!</h2>

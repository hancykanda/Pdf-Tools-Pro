...
export async function POST(request: NextRequest) {
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const qpdf = spawn('qpdf', ['--decrypt', `--password=${password}`, inputPath, outputPath]);
    ...
    return Response.json({ dataUrl, filename: 'unlocked.pdf' });
  } catch (error) {
    console.error('Unlock error:', error);
    return Response.json({ error: 'Failed to unlock PDF. Incorrect password or corrupted file.' }, { status: 500 });
  }
}
